...
public static class ImgGen {
    ...
    public static void test() {
        ...
        var sample = new Bitmap("../../data/Flowers.png");
        ...
        nn.train(1, 1000, inputs.ToArray(), ouputs.ToArray());

        //test
        var output = new Bitmap(sample.Width * 4, sample.Height * 4);

        for (var x = 1; x < output.Width - 1; x++) {
            for (var y = 1; y < output.Height - 1; y++) {

                var kernel = getKernel(output, x, y);
                var result = nn.think(kernel);
                var resultIndex = Util.RandomWeightedChoice(result);
                var color = Color.FromArgb(255, resultIndex, resultIndex, resultIndex);
        ...
        output.Save("../../data/generated.png");
